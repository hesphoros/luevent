...
// 错误信息的加载函数
const char* lu_load_error_string(int error_code) {
    switch (error_code) {
...
unsigned int hash(int error_code) {
...
lu_error_info_t* get_or_create_error_entry(int index)  REQUIRES(error_table_mutex) {
    //unsigned int index = hash(error_code);  // 使用index计算哈希值
...
// 错误表清理
void cleanup_error_table(void) REQUIRES(error_table_mutex)  {
    MutexLocker lock(&error_table_mutex);  // 自动加锁
    for (int i = 0; i < LU_HASH_TABLE_SIZE; i++) {
        if (lu_error_hash_table[i]) {
            free(lu_error_hash_table[i]);  // 释放内存
...
static void initialize_error_table(void) REQUIRES(error_table_mutex) {
    static int initialized = 0;
    MutexLocker lock(&error_table_mutex);  // 自动加锁

    if (!initialized) {
        if (atexit(cleanup_error_table) != 0) {
...
__attribute__((destructor)) void error_table_finalizer(void) NO_THREAD_SAFETY_ANALYSIS  {
    cleanup_error_table();  // 清理哈希表
    
}

// 错误码字符串哈希表访问函数
const char* lu_get_error_string_hash(int errno) REQUIRES(error_table_mutex) ASSERT_CAPABILITY(&error_table_mutex) {
    if (errno < 0 || errno > LU_MAX_ERROR_CODE) {
        return "Unknown error";
    }

    unsigned int index = hash(errno);  // 使用index计算哈希值

    MutexLocker lock(&error_table_mutex);  // 保护哈希表的访问
     
    
    // 获取或创建哈希表条目的指针
    lu_error_info_t* entry = get_or_create_error_entry(index);

    // 如果尚未加载错误信息，则进行加载
    if (!entry->is_loaded) {
        entry->error_code = errno;
        entry->error_message = lu_error_strings_global[errno];
